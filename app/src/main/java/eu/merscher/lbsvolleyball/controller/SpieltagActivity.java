...
public class SpieltagActivity extends AppCompatActivity implements SpieltagActivitySpielerauswahlFragment.OnSpielerClickListenerInFragment {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //getSupportActionBar().setLogo(R.drawable.lbs_volleyball_appicon_white_round_toolbar);
        //Spieltag-Button
        ...
        protected Buchung doInBackground(Void... args) {
            ...
            if (selectedSpieler.size() > 0 && activity.platzkosten > 0) {
                ...
                if (spieler.getHat_buchung_mm() != null) {
                    ...
                    buchungDataSource.createBuchung(spieler.getU_id(), -activity.bu_btr, kto_saldo_alt, kto_saldo_neu, datumsformat.format(kalender.getTime()), "X", null, null);
                    spieler = spielerDataSource.updateTeilnahmenSpieler(spieler);

                } else {

                    double kto_saldo_neu = -activity.bu_btr;
                    buchungDataSource.createBuchung(spieler.getU_id(), -activity.bu_btr, 0, kto_saldo_neu, datumsformat.format(kalender.getTime()), "X", null, null);
